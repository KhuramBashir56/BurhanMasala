...
@php
    $baseClasses = 'whitespace-nowrap flex justify-center items-center rounded-md px-4 py-2 font-medium tracking-wide transition text-center border focus-visible:outline-2 focus-visible:outline-offset-2 active:outline-offset-0 active:opacity-75 disabled:opacity-75 rounded-md cursor-pointer capitalize';
    $colorClasses = [
...
<a {{ $attributes->merge(['class' => $baseClasses . ' ' . $colorClasses[$color], 'title' => $title]) }} x-data="{ loading: false }" x-on:click="loading = true">
    ...
    <div x-show="loading" class="flex justify-center items-center gap-1" style="display: none;">
        <svg xmlns="http://www.w3.org/2000/svg" class="size-5 animate-spin" viewBox="0 -960 960 960" fill="currentColor">
