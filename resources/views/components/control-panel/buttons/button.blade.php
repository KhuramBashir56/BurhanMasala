...
<button {{ $attributes->merge(['class' => $baseClasses . ' ' . $colorClasses[$color], 'title' => $title]) }} wire:loading.attr="disabled" wire:offline.attr="disabled">
    ...
    <div wire:loading wire:target="{{ $target }}" class="flex justify-center items-center gap-1" style="display: none;">
        <svg xmlns="http://www.w3.org/2000/svg" class="size-5 animate-spin inline-flex" viewBox="0 -960 960 960" fill="currentColor">
