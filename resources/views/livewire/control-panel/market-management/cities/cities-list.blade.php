<div class="w-full space-y-4" x-data="">
    ...
    <x-control-panel.table>
        <x-control-panel.table.thead>
            <x-control-panel.table.th content="City Name" />
        ...
        <x-control-panel.table.tbody>
            @forelse ($cities as $city)
                <x-control-panel.table.tr wire:key="city-{{ $city->id }}" class="whitespace-nowrap">
                    <x-control-panel.table.td content="{{ $city->name }}" />
                    <x-control-panel.table.td content="{{ $city->district->name }}" />
                    <x-control-panel.table.td content="{{ $city->district->province->name }}" />
                    @canany(['edit-city'])
                        <x-control-panel.table.actions>
                            @can('edit-city')
                                <x-control-panel.buttons.icon-link wire:navigate href="{{ route('control-panel.market-management.edit-city', ['city' => $city->id]) }}" title="Edit City" color="info">
